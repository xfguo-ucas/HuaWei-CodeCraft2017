...
vector<vector<int>> solveAlgorithm::parseFile(char* topo[MAX_EDGE_NUM], int line_num) {
    ...
    int ServiceLevels = line_num - 1 - result[0][0] - result[0][1] - result[0][2] - 4;
    ...
    // 但是整段的结束符是0x0d
    int index = 2 + ServiceLevels + 1 + result[0][0] + 1;
    int numEdges = result[0][1];    // 边数
    while ( numEdges ) {
        line.clear();
        line.push_back(atoi(strtok(topo[index], " ")));
        line.push_back(atoi(strtok(nullptr, " ")));
        line.push_back(atoi(strtok(nullptr, " ")));
        line.push_back(atoi(strtok(nullptr, " ")));
    ...
    index++;
    int numVertexs = result[0][0];
    line.clear();
    while ( numVertexs ) {
        atoi(strtok(topo[index], " "));
        line.push_back(atoi(strtok(nullptr, " ")));
        index++;
        numVertexs--;
    }
...
vector<int> solveAlgorithm::getServicePosition(Graph& graph, int numServer) {
    // 获取服务器位置函数
    // 注意：要进行均值方差归一化，还有度和带宽的权重设置
    // 注意，返回的result的大小应等于顶点数，如果这个顶点有服务器，那我就为true
    // @黄渲淋负责
    vector<int> result;
    vector<float> eachNodeCanSend(graph.numVertexs, 0);
    vector<int> NodeCost = graph_param[graph_param.size() - 1];
    for (int i = 0; i < graph.numVertexs; i++) {
        //eachNodeCanSend[i] = 0;
        for (int j = 0; j < graph.numVertexs; j++) {
            if (graph.contains[i][j] != MYINFINITY)
                eachNodeCanSend[i] += 1.0 / graph.contains[i][j];   // eachNodeSend[i] 越小越好
        }
    }
    int Allflows = accumulate(eachNodeCanSend.begin(), eachNodeCanSend.end(), 0);
    int AllCosts = accumulate(NodeCost.begin(), NodeCost.end(), 0);
    float alpha = 10;
    for (int i = 0; i < graph.numVertexs; i++) {
        eachNodeCanSend[i] /= Allflows;
        eachNodeCanSend[i] += alpha * (NodeCost[i] / float(AllCosts));
    }
    
    multimap<float, int> temp;
    for (int i = 0; i < graph.numVertexs; i++) {
        temp.insert(make_pair(eachNodeCanSend[i], i));
    }
    
    for (auto i : temp) {
        if (graph.NetNodeNeed.find(i.second) != graph.NetNodeNeed.end())
            continue;
        if (numServer == 0)
            break;
...
int solveAlgorithm::solver(Graph& graph) {
    ...
    int costAllConsumes1 = MYINFINITY;
    
    vector<int> a;
    costAllConsumes = MCF.solve_greedy( graph, graph_param,a);
    if(costAllConsumes!=MYINFINITY)
        costAllConsumes1 = MCF2.solve_DM_greedy( graph, graph_param,a);
    if(costAllConsumes>costAllConsumes1)
        return costAllConsumes1;
    else
...
void return_i(int i,vector<int> stack_r,Graph& graph,vector<float> &flowOfEdges){
    ...
    if(isService(i,graph)){
       ...
       // MCF.serviceSend[i] -= tmp[0];
//        cout<<tmp[2]<<" ";
//        cout<<tmp[1]<<" ";
//        cout<<tmp[0]<<" ";
//        cout<<endl;
        vector<int> tmp1;
        for(int i1=tmp.size()-2;i1>=0;i1--){
            tmp1.push_back(tmp[i1]);
        }
        
        tmp1.push_back(MCF2.servicelevel[tmp1[0]]);
        graph.result.push_back(tmp1);
        return;
    }
    
    
    for(int m=0;m<graph.numVertexs+1;m++){
...
void solveAlgorithm::mergeService(Graph& graph) {
    unordered_map<int, bool> hasmerged;
    int bestCost = MCF.solve(graph, graph_param);
    int cost;
    vector<vector<int>> allPath;
  // MCF.getResult(graph, graph_param);
   // auto tempServiceSend = MCF.serviceSend;
   // sort(tempServiceSend.begin(), tempServiceSend.end());
    
    
    for( int i = 0; i < graph.service_position.size(); i++ ) {
        if (hasmerged.find( graph.service_position[i] ) != hasmerged.end() )
            continue;
        int mindist = MYINFINITY;
        int mergedService1 = -1;
        int mergedService2 = -1;
        for (int j = 0; j < graph.service_position.size(); j++) {
            if (hasmerged.find( graph.service_position[j] ) != hasmerged.end() )
                continue;
            if (i == j)
                continue;
            if (mindist > graph.ShortPathTable[graph.service_position[i]][graph.service_position[j]]) {
//                if (tempServiceSend[graph.service_position[i]] + tempServiceSend[graph.service_position[j]] > 100)
//                    continue;
                mergedService1 = graph.service_position[i];
                mergedService2 = graph.service_position[j];
                mindist = graph.ShortPathTable[mergedService1][mergedService2];
            }
        }
        if (mergedService1 == -1)
            continue;
        
        hasmerged.insert(make_pair(mergedService1, true));
        hasmerged.insert(make_pair(mergedService2, true));
        vector<int> path;   // 最短路径，不要起点终点
        path.push_back(mergedService1);
        int k = graph.PathMatrix[mergedService1][mergedService2];
        while (k != mergedService2) {
            path.push_back(k);
            k = graph.PathMatrix[k][mergedService2];
        }
        path.push_back(mergedService2);
        
        allPath.push_back(path);
    }
    
    // merge操作
    auto best_servicePos = graph.service_position; // merge之后代价如果变高，则不merge
    auto init_servicePos = graph.service_position;
    for (int i = 0; i < allPath.size(); i++) {
        int mergedService1 = allPath[i][0];
        int mergedService2 = allPath[i][allPath[i].size() - 1];
        graph.service_position.erase(find(graph.service_position.begin(), graph.service_position.end(),mergedService1));
        for (int j = 0; j < allPath[i].size(); j++) {
            *(find(graph.service_position.begin(), graph.service_position.end(), mergedService2)) = allPath[i][j];
            cost = MCF.solve(graph, graph_param);
            //cout << cost << endl;
            if (cost < bestCost) {
                cout << "merged " << mergedService1 << " and " << mergedService2 << " : " << j;
                cout << bestCost << "->" << cost << endl;
                bestCost = cost;
                cout << "merge success! " << endl;
                best_servicePos = graph.service_position;
                graph.service_position = init_servicePos;
        ...
        // 更新初始，每次合并都在上次合并结果的基础上合并
        init_servicePos = best_servicePos;
    }
    
    graph.service_position = best_servicePos;
    //sort(graph.service_position.begin(), graph.service_position.end());
    cout << MCF.solve(graph, graph_param) << endl;
}
